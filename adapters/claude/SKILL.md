...
# Natural Voice — Claude Code Adapter (v0.7)

Claude-Code-specific integration for the platform-independent Natural Voice core.
...
This file only explains how Claude Code invokes them. See `docs/adapter-contract.md`
for the universal contract all adapters follow.
...
1. Install the core once (see `docs/installation.md`): `pip install .` from the
   ...
   `PYTHONPATH`.
2. Make this skill discoverable by placing this directory's `SKILL.md` where
   Claude Code looks for project skills (conventionally
   `.claude/skills/natural-voice/SKILL.md` in your project, or the equivalent
   configured skills directory — verify against your Claude Code version).
...
## Workflow (same core, Claude invocation)
...
1. **Profile:** build once from the user's genuine samples —
   ...
   Reuse the saved file afterwards; rebuild only when their writing changes.
2. **Analyze:** run the analyzer on the draft for measurable characteristics.
3. **Compare:** `natural-voice compare --draft draft.txt --profile voice-profile.json`
   and present overall status + confidence, per-dimension findings with
   draft-vs-typical-range numbers, evidence basis, and cautions.
4. **Revise (only when asked):** `natural-voice revise --draft draft.txt
   --profile voice-profile.json --mode conservative` (default). State the mode,
   what changed, and any validation warnings.
5. **Provider:** revision needs an `LLMProvider`. Prefer the provider already
   configured in this Claude session; alternatives come from `providers/`
   (mock for dry runs, openai-compatible, anthropic) via `natural-voice providers`.
   Credentials come from the environment, never files.
...
at `.claude/skills/natural-voice/SKILL.md`.
